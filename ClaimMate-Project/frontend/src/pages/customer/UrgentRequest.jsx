...
const UrgentRequest = () => {
    ...
    useEffect(() => {
        const fetchRepairClaims = async () => {
            ...
            try {
                // ⭐️ (แก้ไข) เรียก API (Route นี้ถูกสร้างใน claimRoute.js ข้อ 11)
    ...
    const handleChange = (e) => {
        ...
        if (files) {
            const selectedFile = files[0] || null; // ⬅️ selectedFile คือ File object
            setFormData(prev => ({ ...prev, file: selectedFile })); // ⬅️ formData.file คือ File object
            if (errors.file) {
    ...
    const handleSubmit = async (e) => {
        ...
        // ⭐️⭐️⭐️ นี่คือบรรทัดที่ต้องแก้ไข ⭐️⭐️⭐️
        // ต้องเป็น formData.file (File Object)
        // ไม่ใช่ formData.file.file (ซึ่งจะเป็น undefined)
        data.append("file", formData.file.file);

        // ⭐️ (เพิ่มตรงนี้เพื่อ Debug) ⭐️
        console.log("Submitting FormData with these values:");
        for (let pair of data.entries()) {
            console.log(pair[0] + ': ', pair[1]);
        }

        try {
            // ⭐️ (แก้ไข) เรียก API ที่ถูกต้อง
            const response = await axios.post(
                "http://localhost:3000/api/claims/urgent-request", // ⬅️ (แก้ไข)
                data,
                {
            ...
            // ✅ (คงเดิม) Handle specific unique error (Backend ส่ง code: 11000 มาให้)
            if (error.response && error.response.data && error.response.data.code === 11000) {
                ...
                // (แสดง error ที่มาจาก 400)
                setErrors(prev => ({
                    ...prev,
                    form: error.response?.data?.message || error.message || "เกิดข้อผิดพลาดในการส่งข้อมูล"
                }));
            }
        }
    };
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2">
                    <form onSubmit={handleSubmit} className="card space-y-6">
                            ...
                            >
                                ...
                                <option value="">
                                    {repairClaims.length > 0
                                        ? '-- เลือกการเคลม --'
                                        : (customerID ? 'ไม่พบเคลมที่เข้าเกณฑ์ (กำลังซ่อม)' : 'กำลังโหลดข้อมูล...')}
                                </option>
                        ...
                        {/* ... (โค้ดส่วนที่เหลือของ Form (Select Reason, Description, File Upload, Submit) ไม่มีการเปลี่ยนแปลง) ... */}
                ...
                {/* ... (โค้ดส่วน Sidebar และ Modal ไม่มีการเปลี่ยนแปลง) ... */}
