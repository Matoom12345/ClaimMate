...
const ClaimDetail = () => {
  ...
  const getGarageDisplay = () => {
    // ⭐️ (แก้ไข) โค้ดเดิมของคุณ (บรรทัด 203) ใช้ 'claim.currentStep'
    // เราจะใช้ 'claim.currentStep' (ที่เป็น 'reported', 'inspected') ต่อไป

    // ขั้นตอนที่ไม่ควรแสดงอู่: reported, inspected, approved
    const stepsWithoutGarage = ['reported', 'inspected']; // ⭐️ (Approved ควรแสดงอู่)

    // ⭐️ (ใช้ claim.currentStep ที่เรา map ค่าแล้ว)
    ...
    if (claim.currentStep === 'approved' && !claim.garage) { // ⭐️ (เปลี่ยนจาก garage_selected เป็น approved)
                    ...
                    {claim.images.map((image, index) => (
                        ...
                        >
                          ...
                          <div className={`absolute top-2 left-2 text-white text-xs px-2 py-1 rounded-full flex items-center gap-1 ${
                              image.type === 'document' ? 'bg-info' : 'bg-error'
                          }`}>
          ...
          <div className="space-y-6">
                ...
                <span className="material-icons-round text-primary-500 leading-none">
                  ...
                  {garageDisplay.type === 'action' && (
                      <div className="text-center py-4">
                        ...
                        <Link
                            to={`/customer/claims/${claim.claimNumber}/select-garage`} // ⭐️ (ใช้ claimNumber)
            ...
            <div className="card">
              ...
              <div className="space-y-2">
                ...
                {/* ⭐️ (ใช้ claim.currentStep ที่ map ค่าแล้ว) */}
                {claim.currentStep === "approved" && !claim.garage && (
                    <Link
                        to={`/customer/claims/${claim.claimNumber}/select-garage`} // ⭐️ (ใช้ claimNumber)
