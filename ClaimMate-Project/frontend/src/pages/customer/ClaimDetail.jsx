...
const ClaimDetail = () => {
  ...
  const loadClaimData = async () => {
    try {
      ...
      // ✅ ข้อ 4: รูปภาพจาก backend (ถ้ามี)
      const images = c.images || []; // TODO: Backend - ดึงรูปภาพจาก database

      // ✅ ข้อ 5: รายการซ่อม (สำหรับกรณีอนุมัติไม่สำเร็จ)
      const items = c.repairItems || [
        { id: 1, name: 'ซ่อมบังโคลนข้าง', cost: 52000 },
        { id: 2, name: 'เปลี่ยนกับหลัง', cost: 4000 },
      ]; // TODO: Backend - ดึงรายการซ่อมจาก database
      ...
      setClaim({
        ...
        // title: c.title, // ❌ ลบ title ตามข้อ 1
        detail: c.detail,
        location: c.location,
        status: c.state,
        currentStep: c.currentStep,
        priority: c.priority || 'normal', // ✅ เพิ่ม priority

        incidentDate: c.incidentDate,
        estimatedCost: c.estimatedCost || 0,
        approvedAmount: c.approvedCost || 0,
        deductible: c.additionalCost || 0,
        insuranceCoverage: c.insuranceCoverage || 25000, // ✅ ข้อ 5: วงเงินประกัน
        ...
        // ✅ ข้อ 5: สถานะเคลมไม่สำเร็จ
        approvalFailed: c.approvalFailed || false,
        exceedsBudget: c.exceedsBudget || false,
      });
  ...
  const getGarageDisplay = () => {
    // ขั้นตอนที่ไม่ควรแสดงอู่: reported, inspected, approved
    const stepsWithoutGarage = ['reported', 'inspected', 'approved'];

    if (stepsWithoutGarage.includes(claim.currentStep)) {
      return { show: true, type: 'placeholder', message: '-' };
    }

    // ขั้นตอน garage_selected: แสดง "เลือกอู่ซ่อม" ถ้ายังไม่เลือก
    if (claim.currentStep === 'garage_selected' && !claim.garage) {
      return { show: true, type: 'action', message: 'เลือกอู่ซ่อม' };
    }

    // ถ้ามีอู่แล้ว แสดงข้อมูลอู่
    if (claim.garage) {
...
const handleAcceptExtraCost = async () => {
  ...
  try {
    // TODO: Backend - บันทึกการยืนยัน และอัปเดตสถานะ
    await axios.post(`http://localhost:3000/api/claims/${id}/accept-extra-cost`, {
      extraCost: getExtraCost()
    });
    
    // ✅ อัปเดต UI ทันที (ไม่ต้องรอ reload)
    setClaim(prev => ({
      ...prev,
      deductible: getExtraCost(), // ✅ เพิ่มค่าใช้จ่ายเพิ่มเติม
      status: 'approved', // ✅ เปลี่ยนสถานะเป็น "อนุมัติเคลม"
      approvedAmount: getTotalRepairCost() - getExtraCost(), // ✅ จำนวนเงินที่อนุมัติ = ยอดรวม - ส่วนเกิน
    }));
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          {/* ✅ ข้อ 1: แสดง priority badge แทนเลขที่เคลม */}
          <div className="flex items-center gap-2">
            <p className="text-neutral-500">ระดับความด่วน:</p>
      ...
      {/* ✅ ข้อ 5: Alert สำหรับกรณีอนุมัติไม่สำเร็จ */}
      {claim.approvalFailed && claim.exceedsBudget && (
        <div className="card bg-warning-50 border-2 border-warning">
          <div className="flex items-start gap-4">
            <span className="material-icons-round text-warning text-3xl">
              warning
            </span>
            <div className="flex-1">
              <h3 className="font-semibold text-warning-dark mb-2 text-lg">
                อนุมัติเคลมไม่สำเร็จ
              </h3>
              <p className="text-neutral-700 mb-4">
                กรุณายืนยันความยินยอมรับผิดชอบส่วนเกินวงเงิน
              </p>

              <div className="bg-white p-4 rounded-lg mb-4">
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    <p className="text-neutral-500">ค่าซ่อมประเมิน</p>
                    <p className="font-bold text-neutral-dark text-lg">
                      ฿{claim.estimatedCost.toLocaleString()}
                    </p>
                  </div>
                  <div>
                    <p className="text-neutral-500">วงเงินคุ้มครอง</p>
                    <p className="font-bold text-primary-600 text-lg">
                      ฿{claim.insuranceCoverage.toLocaleString()}
                    </p>
                  </div>
                  <div className="col-span-2">
                    <p className="text-neutral-500">ส่วนเกินที่ต้องจ่าย</p>
                    <p className="font-bold text-error text-xl">
                      ฿{(claim.estimatedCost - claim.insuranceCoverage).toLocaleString()}
                    </p>
                  </div>
                </div>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          ...
          {/* ✅ ข้อ 4: รูปภาพความเสียหายและเอกสารประกอบ */}
          <div className="card-static">
            <h2 className="text-xl font-semibold text-neutral-dark mb-4 flex items-center gap-2">
              <span className="material-icons-round text-primary-500 leading-none">
                photo_library
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                {claim.images.map((image, index) => (
                  ...
                  >
                    ...
                    <div className="absolute top-2 left-2 bg-error text-white text-xs px-2 py-1 rounded-full flex items-center gap-1">
          ...
          {/* ✅ รายการซ่อม (Read-only สำหรับลูกค้า) */}
          <div className="card-static">
            <h2 className="text-xl font-semibold text-neutral-dark mb-4 flex items-center gap-2">
              <span className="material-icons-round text-primary-500 leading-none">
                build
                ...
                <div className="space-y-2 pt-4 border-t">
                  <div className="flex justify-between items-center text-sm">
                    ...
                    <span className="font-semibold text-primary-600">
                      ฿{(claim?.insuranceCoverage || 0).toLocaleString()}
                    </span>
                  </div>
        ...
        <div className="space-y-6">
          ...
          {/* ✅ ปุ่มดำเนินการเมื่อเกินวงเงิน */}
          {repairItems.length > 0 && getExtraCost() > 0 && (
            <div className="card bg-warning-50 border-2 border-warning">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-neutral-700 text-sm mb-3">
                    ค่าซ่อมเกินวงเงินคุ้มครอง ฿{getExtraCost().toLocaleString()}
                  </p>
          ...
          {/* ✅ ข้อ 2: Garage Info - แสดงตาม logic ที่กำหนด */}
          {garageDisplay.show && (
            <div className="card-static">
              <h3 className="font-semibold text-neutral-dark mb-4 flex items-center gap-2">
                <span className="material-icons-round text-primary-500 leading-none">
          ...
          <div className="card">
            ...
            <div className="space-y-2">
              {/* ✅ ข้อ 3: ปุ่มเลือกอู่ซ่อม (แสดงเมื่อ approved และยังไม่มีอู่) */}
            ...
            <div className="bg-neutral-50 p-4 rounded-lg">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-medium text-neutral-dark">
                    {selectedImage.type || 'ความเสียหาย'}
                  </p>
                </div>
                <div>
